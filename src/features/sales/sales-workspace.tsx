...
function OfferDetailView({ customers, members, initialQuote, onBack, onSubmit, onResponsibleChange, onArchive }: { customers: Customer[]; members: Array<{ id: string; displayName: string }>; initialQuote?: Quote; onBack: () => void; onSubmit: (form: FormData) => Promise<void>; onResponsibleChange: (quoteId: number, responsibleId: string) => void; onArchive: (quoteId: number) => void }) {
  const [tab, setTab] = useState<OfferTab>("general"); const [saving, setSaving] = useState(false); const [documentToDelete, setDocumentToDelete] = useState<AttachedDocument | null>(null); const [archiveConfirmation, setArchiveConfirmation] = useState(false); const [documents, setDocuments] = useState<AttachedDocument[]>(initialQuote ? [{ id: "scope", name: "Alcance técnico", fileName: `Alcance-Q-${initialQuote.quoteNumber}.pdf`, fileSize: "1.8 MB" }] : []); const isNew = !initialQuote;
