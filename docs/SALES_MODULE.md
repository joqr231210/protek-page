...
The Kanban board uses draggable offer cards, full-column drop targets, and a highlighted target during drag. Its stage move still uses `move_sales_offer_stage`. Settings follow `Ajustes > Ventas > Ofertas > Estatus de ofertas`; the last level owns the five-stage name, color, and visibility controls. Breadcrumb steps are clickable.

An existing offer's colored stage chip also changes its stage without replacing the open form. The overflow menu archives an offer, restores an archived offer, or lets a Sales administrator permanently delete it after confirmation. Archived offers remain in the Offers table with an `Archivada` status and can be filtered there; they are read-only until restored and excluded from the board and active pipeline. The table keeps its column headings without a redundant outer title. Settings can save a custom hexadecimal color for each of the five visible-stage candidates, reused in the chip, table, board, and Summary. The Summary funnel shows visible, non-closed stages with an offer count and sums grouped by currency, rather than silently adding unlike currencies.
